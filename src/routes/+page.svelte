<script>
  ...
  const translations = {
    en: {
      ...
      legalError:
        "Please accept the Privacy Policy and Terms and Conditions to continue.",
      ...
      installHelpIOS:
        'To save Joyering on your phone, tap Share, then tap "Add to Home Screen".',
      installHelpAndroid:
        'To save Joyering on your phone, open the browser menu and choose "Install app" or "Add to Home screen".',
      installHelpGeneric:
        "To save Joyering on your phone, open your browser menu and look for an install option.",
    ...
    it: {
      ...
      sendingLoginLink: "Invio in corso...",
      loginCheckEmail:
      ...
      legalError:
        "Per favore, accetta l’Informativa sulla Privacy e i Termini e Condizioni per continuare.",
      ...
      installCardText:
        "Aggiungi Joyering al telefono per averlo sempre a portata di mano.",
      installButton: "Installa Joyering",
      notNow: "Non ora",

      installHelpIOS:
        'Per salvare Joyering sul tuo telefono, tocca Condividi e poi "Aggiungi alla schermata Home".',
      installHelpAndroid:
        'Per salvare Joyering sul tuo telefono, apri il menu del browser e scegli "Installa app" oppure "Aggiungi alla schermata Home".',
      installHelpGeneric:
        "Per salvare Joyering sul tuo telefono, apri il menu del browser e cerca l’opzione per installare l’app.",
    ...
    pt: {
      ...
      legalError:
        "Por favor, aceite a Política de Privacidade e os Termos e Condições para continuar.",
      ...
      installCardText:
        "Adicione Joyering ao celular para ter acesso fácil a qualquer momento.",
      installButton: "Instalar Joyering",
      notNow: "Agora não",

      installHelpIOS:
        'Para salvar o Joyering no seu celular, toque em Compartilhar e depois em "Adicionar à Tela de Início".',
      installHelpAndroid:
        'Para salvar o Joyering no seu celular, abra o menu do navegador e escolha "Instalar app" ou "Adicionar à tela inicial".',
      installHelpGeneric:
        "Para salvar o Joyering no seu celular, abra o menu do navegador e procure a opção de instalar o app.",
  ...
  function saveLegalAcceptanceForEmail(value) {
    ...
    if (!emails.includes(normalized)) {
      ...
      localStorage.setItem(
        "joyering-legal-accepted-emails",
  ...
  async function login() {
    ...
    const redirectUrl =
      typeof window !== "undefined"
        ? `https://app.joyering.com?lang=${language}`
        : "";
  ...
  function playRandomTapSound() {
    ...
    const sound = /** @type {HTMLAudioElement} */ (
      tapSounds[index].cloneNode()
    );
    sound.volume = 0.12;
    sound.play().catch(() => {});
  }
  ...
  async function ensureJoyStateRow() {
    ...
    if (!data) {
      const { error: insertError } = await supabase.from("joy_state").insert({
        user_id: user.id,
        butterfly_count: 0,
      });
  ...
  <div class="auth-page">
    <div class="auth-card">
      ...
      <div class="auth-form">
          ...
          <label class="legal-consent">
            ...
            <span>
              {#if language === "it"}
                Accetto l’
                <a
                  href="https://joyering.com/it/privacy-policy"
                  target="_blank"
                  rel="noopener noreferrer">{t("privacyLabel")}</a
                >
                e i
                <a
                  href="https://joyering.com/it/terms-and-conditions"
                  target="_blank"
                  rel="noopener noreferrer">{t("termsLabel")}</a
                >.
              {:else if language === "pt"}
                Concordo com a
                <a
                  href="https://joyering.com/pt/privacy-policy"
                  target="_blank"
                  rel="noopener noreferrer">{t("privacyLabel")}</a
                >
                e os
                <a
                  href="https://joyering.com/pt/terms-and-conditions"
                  target="_blank"
                  rel="noopener noreferrer">{t("termsLabel")}</a
                >.
              {:else}
                I agree to the
                <a
                  href="https://joyering.com/privacy-policy"
                  target="_blank"
                  rel="noopener noreferrer">{t("privacyLabel")}</a
                >
                and
                <a
                  href="https://joyering.com/terms-and-conditions"
                  target="_blank"
                  rel="noopener noreferrer">{t("termsLabel")}</a
                >.
              {/if}
        ...
        <button on:click={login} disabled={!email.trim() || isLoggingIn}>
          {isLoggingIn ? t("sendingLoginLink") : t("sendLoginLink")}
        </button>
...
{:else}
  <div class={`app-shell theme-${theme}`}>
            ...
            {#each categories as category}
              ...
              >
                ...
                <span class="category-label">
                  {translations[language].categories[category.key]}
                </span>
              </button>
            {/each}
          </div>
        ...
        {:else}
          <div class="collection-screen">
            ...
            {:else}
              ...
              <p class="subtitle collection-subtitle">
    ...
    {#if isSettingsOpen}
      <div class="settings-overlay" on:click={() => (isSettingsOpen = false)}>
        <div class="settings-modal" on:click|stopPropagation>
          ...
          <button class="settings-logout" type="button" on:click={logout}>
            {t("logout")}
          </button>
        </div>
      </div>
